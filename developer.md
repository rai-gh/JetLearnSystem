...
## 外部とのデータの入出力
JetLearnSystem（JLS）では、システムの状態保存や演習用データの管理にJSON形式のファイルを採用しています。本ページでは、それぞれのデータの役割と、詳細なJSONデータ構造について解説します。

1. **ユーザー進捗データ**（外部ファイルへの入出力）
学習者の進捗状況など、動的に変化する状態データは、アプリケーションの実行ディレクトリ（JARファイルと同階層）に外部ファイルとして保存します。

セーブデータ (continue.json)
演習モードを中断・再開するためのデータです。

入出力処理:

出力: ユーザーが「保存して終了」アクションを実行した際に、現在の進捗をJSON形式で書き出します。

入力: 演習モード再開時に、該当ファイルが存在するか確認し、読み込むことで前回の状態を復元します。

データ構造 (JSON):
ユーザーが最後に到達した問題のIDを保持します。

```JSON
{
  "last_problem_id": "現在の問題のID（文字列または数値）"
}
```

2. **演習問題・システム定義データ**（内部リソースからの入力）
演習問題のデータや全体の遷移ルールは、ファイルの欠損や改ざんを防ぐため、外部ファイルではなく「JARファイル内に内包するリソース」として扱います。これらは読み込み専用（Read-Only）として機能します。

2.1. **問題定義ファイル**
各演習問題の具体的な内容を定義するファイルです。

配置場所: /questions/json/ 配下

データ構造 (JSON):
1つの問題につき、以下の5つの要素を保持します。

```JSON
{
  "id": "問題番号または一意のID",
  "text": "問題文のテキスト",
  "image": "問題として提示する画像のファイルパス（/questions/image/配下など）",
  "model_code": "模範解答となるJET言語のコード",
  "model_image": "模範解答実行時の完成図（画像ファイルパス）"
}

2.2. **演習構造定義ファイル** (exercise_structure.json)
演習モード全体のフロー（問題の提示順序や分岐ルール）を管理するファイルです。

配置場所: /questions/exercise_structure.json

データ構造 (JSON):
演習の開始地点と、ユーザーの正誤判定に応じた次の問題への遷移ルールを定義します。

```JSON
{
  "start_problem_id": "最初に提示する問題のID",
  "transitions": {
    "問題ID_1": {
      "on_correct": "正解時に遷移する次の問題ID",
      "on_incorrect": "不正解時に遷移する次の問題ID"
    },
    "問題ID_2": {
      "on_correct": "正解時に遷移する次の問題ID",
      "on_incorrect": "不正解時に遷移する次の問題ID"
